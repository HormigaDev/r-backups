...
pub async fn migration_down(migration_id: &str, dbname: &str, groupdb: &str) {
    ...
    let down_sql = extract_migration_section(&migration_content, false);
    ...
    if !dbname.is_empty() {
        ...
        if !applieds.contains(&migration_id.to_string()) {
            ...
            applieds.retain(|x| x != migration_id);
    ...
    if !groupdb.is_empty() {
        ...
        for db_name in databases {
            ...
            if let Err(err) = new_db.client.batch_execute(&down_sql).await {
